...
<body class="bg-slate-900">
    <?php
        ...
        if(isset($_POST["submit"])){
            //Procedural MySQLi is a double-edged sword. It feels more natural, until it isn't. Enjoy this catastrophe about to unfold.
    ...
    <main class="grid w-full h-screen px-4 place-items-center">
        <div class="grid grid-cols-2 border border-slate-50/[0.06] rounded-md overflow-hidden">
            ...
            <form method="post" class="flex flex-col p-10">
                <?php
                    if(!isset($row) && isset($_POST["submit"])) {
                     ...
                     </svg> User not found.</p>';
                    }
                    if(isset($row) && isset($_POST["submit"]) && !password_verify($password, $row["password"])){
                        echo '<p class="text-violet-400 mb-4 flex items-center gap-1"><svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-alert-circle" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                        <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                        <path d="M3 12a9 9 0 1 0 18 0a9 9 0 0 0 -18 0"></path>
                        <path d="M12 8v4"></path>
                        <path d="M12 16h.01"></path>
                     </svg> Incorrect password</p>';
                    }
                ?>
                <fieldset class="flex flex-col-reverse">
                    ...
                    ?> placeholder=" " id="username" name="username" required type="text" class="bg-transparent border-b outline-none border-slate-50/[0.06] placeholder-shown:h-0 h-5 focus:h-5 duration-200 peer text-slate-400 focus:text-violet-400 pb-1"/>
                ...
                <button type="submit" name="submit" class="p-2 mt-4 rounded bg-violet-400/10 text-violet-400 hover:bg-violet-400/20">Login</button>
